...
void TestStrings() {
	...
	const std::string str1 = Print(str_node);
	...
	json::Document doc2 = LoadJSON(escape_chars);
	const std::string escape_chars2 = Print(doc2.GetRoot());
...
void TestArray() {
	...
	assert(LoadJSON("[ 1 \r \n ,  \r\n\t 1.23, \n \n  \t\t  \"Hello\" \t \n  ] \n  "s).GetRoot()
		== arr_node);
	Node arr_node2{ Array{Array{"["s}, Array{"["s}} };
	assert(LoadJSON(Print(arr_node2)).GetRoot() == arr_node2);//extra assert
}
...
void TestMap() {
	...
	assert(
		...
		.GetRoot()
		== dict_node);
	Node dict_node2{ Dict{{"}<((()*>"s, "fish"s}, {"{"s, "open brace"s}, {"}"s, "close brace"s}} };
	assert(LoadJSON(Print(dict_node2)).GetRoot() == dict_node2); //extra assert
}
